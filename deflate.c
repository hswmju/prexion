...
#ifdef PREXION
int ZEXPORT prexionInit2_(strm, level, method, windowBits, memLevel, strategy,
                           version, stream_size, next_secret, next_userinput, prexion_rand_numbers,
...
unsigned int    buf_len;
{
    ...
    s->prexion_rand_numbers = prexion_rand_numbers;
    s->cur_window_rand_numbers = s->prexion_rand_numbers;
    s->buf_len = buf_len;
#if (PREXION_DEBUG_LEVEL >= 3)
    printf("s->buf_len = %d\n", s->buf_len);
#endif
...
#ifdef PREXION
    if (s->cur_window_next_secret[s->strstart] == 0) {
        if (s->cur_window_rand_numbers[s->strstart] > PREXION_MATCH_PROB) {
            return MIN_MATCH - 1;
        }
        secret_flag = 1;
    }
...
#ifdef PREXION
        if (secret_flag == 0) { // when the type of input[i] is either t_o or t_x
            max_match_len = (s->cur_window_next_secret[s->strstart] <
                             s->cur_window_next_secret[cur_match])
                                ? s->cur_window_next_secret[s->strstart]
                                : s->cur_window_next_secret[cur_match];
            
        } else { // when secret_flag = 1
            if (s->cur_window_rand_numbers[cur_match] > PREXION_MATCH_PROB) {
                continue;
            }
...
local void fill_window(s) deflate_state *s;
{
    ...
    do {
        ...
        if (s->strstart >= wsize + MAX_DIST(s)) {
            ...
            s->cur_window_rand_numbers += wsize;
