...
#ifdef PREXION
void gz_compress      OF((FILE   *in, gzFile out, unsigned short *next_secret, unsigned short *next_userinput, unsigned char *prexion_rand_numbers, unsigned int buf_len));
#else
...
#ifdef PREXION
void file_compress    OF((char  *file, char *mode, unsigned short *next_secret, unsigned short *next_userinput, unsigned char *prexion_rand_numbers, unsigned int buf_len));
#else
 ...
 */
#ifdef PREXION
void gz_compress(in, out, next_secret, next_userinput, prexion_rand_numbers, buf_len)
    FILE   *in;
    gzFile out;
    ...
    unsigned int buf_len;
{
    ...
    for (;;) {
        ...
        if (gzwrite(out, buf, (unsigned)len, next_secret, next_userinput, prexion_rand_numbers, buf_len) != len) error(gzerror(out, &err));
    }
    fclose(in);
 ...
 */
#ifdef PREXION
void file_compress(file, mode, next_secret, next_userinput, prexion_rand_numbers, buf_len)
    char  *file;
    char  *mode;
    unsigned short *next_secret;
    unsigned short *next_userinput;
    ...
    unsigned int buf_len;
{
    ...
    gz_compress(in, out, next_secret, next_userinput, prexion_rand_numbers, buf_len);
}
#else
...
#ifdef PREXION
                        gz_compress(in, file, next_secret, next_userinput, prexion_rand_numbers, next_secret_len);
#else
                        gz_compress(in, file);
#endif
                    }
...
#ifdef PREXION
                    file_compress(*argv, outmode, next_secret, next_userinput, prexion_rand_numbers, next_secret_len);
#else
